...
public class BuskingManager : MonoBehaviour
{
    ...
    SongDataBusking m_currentSongBusking;
    ...
    private string selectedChorusBusking;
    private string selectedChorusPathBusking;     
	...
	{
		m_audioSource.Stop ();
		m_currentSongBusking.clip = music;
		m_audioSource.Play ();
	}

    private void InitGame()
    {
        BuskingMusicInitialization ();
        ChartInitialization();
        SpawnSoloSections();
        PlayMusic();
    ...
    //Andys Shit
    private void BuskingMusicInitialization() 
	{
		m_currentSongBusking = GameManager_Data.Instance.selectedSongBusking;
		if (GetComponent<AudioSource>() == false)
			m_audioSource = gameObject.AddComponent<AudioSource>();

		m_audioSource.clip = m_currentSongBusking.clip;
	}
    ...
    private void ChartInitialization()
    {
		...
		Chart selectedSongBuskingChart = chartReaderBase.ReadChartFile (Application.streamingAssetsPath + "/charts/" + m_currentSongBusking.songName + "/" + m_currentSongBusking.baseChartPath);
		...
		//this is it
		Chart selectedChorusBuskingChart = chartReaderChorus.ReadChartFile(Application.streamingAssetsPath + "/Charts/" + m_currentSongBusking.songName + "/" + selectedChorusPathBusking);
        ...
        Note[] selectedSongBuskingNotes = selectedSongBuskingChart.GetNotes(selectedSongDifficulty);
        Note[] selectedChorusBuskingNotes = selectedChorusBuskingChart.GetNotes(selectedSongDifficulty);
    ...
    {
		foreach (SongDataBusking.ChorusSections solo in m_currentSongBusking.soloSections)
        {
